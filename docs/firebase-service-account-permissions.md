...
MVPでは、PCブリッジ用に専用service accountを作成し、Firestore read/writeに必要な `roles/datastore.user` と、Firebase Storage objectのread/writeに必要な最小限の権限を付与する運用を推奨する。
...
- Firebase Storageには、object read/writeに必要な権限だけを付与する。初期運用では `roles/storage.objectUser` を使い、必要に応じてbucket単位またはcustom roleへ絞る。
