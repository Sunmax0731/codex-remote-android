...
- Codex処理後に生成された画像は入力添付とは別に `resultAttachments` として扱う。AndroidからPCへ渡す `attachments` と、PCからAndroidへ返す `resultAttachments` を混同しない。
...
- Android writes `id`, `type`, `fileName`, `contentType`, `sizeBytes`, `storagePath`, and `sha256`.
- PC bridge derives the Codex disposition from `type`: `image` becomes `--image`, and `file` becomes `--add-dir` plus a prompt reference.
- Local PC paths are never written back to Firestore or notification payloads.

## Result attachment model

PCブリッジが最終結果本文のMarkdown画像参照からローカル画像を検出した場合、画像をFirebase Storageへuploadし、同じcommand documentへ `resultAttachments` 配列を保存する。

```json
{
  "resultText": "画像を生成しました。\n\n![result](image.png)",
  "resultAttachments": [
    {
      "id": "result_01",
      "type": "image",
...
`resultAttachments` は出力専用metadataであり、Androidアプリは読み取りだけ行う。PCブリッジはremote URL、data URL、存在しないローカルpath、画像以外の参照をupload対象にしない。
...
- `results` pathはAndroid userがreadできるが、client writeは許可しない。PCブリッジがAdmin SDK credentialでuploadする。
...
5. `type=image` は `codexImages` と合流し、`--image` に渡す。
6. `type=file` はattachment directoryを `--add-dir` に渡し、prompt末尾へ参照path一覧を追加する。
7. Codexの最終結果本文にMarkdown画像参照が含まれる場合、参照先ローカル画像をStorageの `results` pathへuploadし、`resultAttachments` を保存する。
8. command完了後、local attachment cacheを削除する。失敗時も次回起動時に古いcacheをcleanupする。

## Security and limits

- MVP初期値は1 commandあたり最大5 files、合計25MBを目安にする。
- 実装上限は1 commandあたり5 files、1 fileあたり25 MiB。
...
- `.exe`, `.bat`, `.cmd`, `.ps1`, `.sh`, archiveなどは初期実装では選択・metadata作成・Rules通過の対象にしない。
...
- result画像はStorage objectとして保存し、Firestoreにはmetadataだけを保存する。
