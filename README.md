...
Android app <- Firebase Storage result images <- PC bridge
...
- 最終結果またはエラー表示。
- 最終結果に画像が含まれる場合のサムネイル表示、タップでの拡大表示、ロングタップでの保存。
...
利用者自身のFirebaseプロジェクトを作成し、このリポジトリの `firebase/` 構成をデプロイして使います。Firestore Rules、Indexes、Storage Rules、Cloud Functionsをデプロイする場合は次を実行します。
...
`firebase use --add` では、利用者自身が用意したFirebase project IDを選択します。画像・ファイル添付と結果画像表示にはFirebase Storage bucketが必要です。新しいFirebaseプロジェクトでは `<project-id>.firebasestorage.app` 形式のbucket名を使う場合があります。Cloud Functionsの初回デプロイにはBlazeプランとGoogle Cloud APIの有効化が必要です。詳細は [Firebase手順](firebase/README.md) と [Cloud Functions手順](firebase/functions/README.md) を参照してください。
...
- 進捗概要
- 最終結果またはエラー
- 入力添付と結果画像のmetadata、Storage path、hash
