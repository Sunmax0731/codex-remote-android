...
```json
{
  ...
  "firebaseStorageBucket": "your-firebase-project-id.firebasestorage.app",
  "serviceAccountPath": "D:\\secure\\codex-remote-service-account.json",
  "attachmentCachePath": ".local\\attachments",
  "pollIntervalSeconds": 5,
  "heartbeatIntervalSeconds": 300,
  "maxCommandsPerTick": 5
}
...
- 入力添付をFirebase Storageからdownloadし、画像はCodex CLIの `--image`、汎用ファイルは `--add-dir` とprompt参照へ渡す。
- 最終結果本文のローカルMarkdown画像参照を検出し、画像をFirebase Storageの `/results/` pathへuploadして `resultAttachments` metadataを書き戻す。

TypeScript変更後は `npm.cmd run build` だけでなく、起動済みの `start:watch` プロセスを再起動する。常駐watcherは起動時点の `dist` を使うため、再起動しないと結果画像uploadなどの修正が反映されない。
