...
export function normalizeConfig(raw: RawConfig): BridgeConfig {
  ...
  return {
    ...
    firebaseStorageBucket: normalizeOptionalString(raw.firebaseStorageBucket),
    serviceAccountPath: raw.serviceAccountPath,
    attachmentCachePath: raw.attachmentCachePath ?? ".local/attachments",
    relayMode,
    localRelayPath: raw.localRelayPath ?? ".local/relay-state.json",
    claimTtlSeconds: raw.claimTtlSeconds ?? 300,
    pollIntervalSeconds: raw.pollIntervalSeconds ?? 5,
    heartbeatIntervalSeconds: raw.heartbeatIntervalSeconds ?? 300,
