  ...
  async invoke(input: CodexInvocation): Promise<CodexInvocationResult> {
    ...
    try {
      ...
      if (result.exitCode !== 0) {
        return {
          kind: "failure",
          errorText: result.stderr.trim() || `Codex CLI failed with exit code ${result.exitCode}.`,
        };
      }
      ...
      return {
        kind: "success",
        resultText: output.trim() || result.stdout.trim() || "Codex CLI completed without a final message.",
      };
    } finally {
